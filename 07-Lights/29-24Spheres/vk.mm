...
NSString *gszWindowTitle = @"macOS: 24 Spheres - Vulkan";
...
- (VkResult) initialize {
    ...
    // Create Index Buffer
    vkResult = [self createIndexBuffer];
    if(vkResult != VK_SUCCESS) {
...
- (VkResult) createDescriptorSetLayout {
    ...
    vkDescriptorSetLayoutBinding.stageFlags = VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT; // this binding is used in both vertex and fragment shaders
...
- (VkResult) buildCommandBuffers {
    ...
    for(uint32_t i = 0; i < swapchainImageCount; i++) {
        ...
        float fbWidth = (float)vkExtent2D_swapchain.width;
        float fbHeight = (float)vkExtent2D_swapchain.height;

        float aspect = fbWidth / fbHeight;
        float gridMargin = 0.85f; // fraction of the window the grid should occupy

        float cellHeightFromRows = (fbHeight * gridMargin) / rows;
        float cellHeightFromCols = (fbWidth * gridMargin) / cols / aspect;
        float cellHeight = MIN(cellHeightFromRows, cellHeightFromCols);
        float cellWidth = cellHeight * aspect;

        float gridWidth = cols * cellWidth;
        float gridHeight = rows * cellHeight;

        float startX = (fbWidth - gridWidth) / 2.0f;
        float startY = (fbHeight - gridHeight) / 2.0f;
        ...
        for(uint32_t k = 0; k < cols; k++) {
            for(uint32_t j = 0 ; j < rows; j++) {
                ...
                vkViewport.x = startX + (k * cellWidth);
                vkViewport.y = startY + (j * cellHeight);
                vkViewport.width = cellWidth;
                vkViewport.height = cellHeight;
