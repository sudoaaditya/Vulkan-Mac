...
NSString *gszWindowTitle = @"macOS: Tessellation Shader - Vulkan";
...
- (VkResult) getPhysicalDevice {
    ...
    if(vkPhysicalDeviceFeatures.fillModeNonSolid == VK_TRUE) {
        fprintf(fptr, "getPhysicalDevice(): Selected Physical Device Supports Fill Mode Non Solid (wireframe)!.\n");
    } else {
        fprintf(fptr, "getPhysicalDevice(): Selected Physical Device Does Not Support Fill Mode Non Solid (wireframe)!.\n");
    }
...
- (VkResult) createVulkanDevice {
    ...
    if(vkPhysicalDeviceFeatures.tessellationShader == VK_TRUE) {
        ...
        vkResult = VK_ERROR_INITIALIZATION_FAILED;
        fprintf(fptr, "createVulkanDevice(): Selected Physical Device Does Not Supports Tessellation Shader!.\n");
        return (vkResult);
    }

    // ! For Wireframe Rendering (used since MoltenVK/Metal has no isolines tessellation domain)
    if(vkPhysicalDeviceFeatures.fillModeNonSolid == VK_FALSE) {
        vkResult = VK_ERROR_INITIALIZATION_FAILED;
        fprintf(fptr, "createVulkanDevice(): Selected Physical Device Does Not Support Fill Mode Non Solid!.\n");
        return (vkResult);
    }
...
- (VkResult) createVertexBuffer {
    ...
    if(bFoundMatchingMemoryType_vertex == VK_FALSE) {
        vkResult = VK_ERROR_OUT_OF_DEVICE_MEMORY;
        fprintf(fptr, "createVertexBuffer(): Failed to find suitable memory type for Vertex Buffer!.\n");
        return (vkResult);
    } else {
        fprintf(fptr, "createVertexBuffer(): Suitable memory type found for Vertex Buffer!.\n");
    }
...
- (VkResult) updateUniformBuffer {
    ...
    NSString *windowTitle = [NSString stringWithFormat:@"AMK_Vulkan : Tessellation Shader : [ Segments :: %d ]", numLineSegments];
...
- (VkResult) createShaders {
    ...
    fp = fopen(pszFileName, "rb");
    if(fp == NULL) {
        fprintf(fptr, "createShaders(): fopen() failed to open Tessellation Evaluation Shader spir-v file!.\n");
        vkResult = VK_ERROR_INITIALIZATION_FAILED;
        return (vkResult);
    } else {
        fprintf(fptr, "createShaders(): fopen() succeed to open Tessellation Evaluation Shader spir-v file!.\n");
    }
    ...
    retVal = fread(shaderData, fileSize, 1, fp);
    if(retVal != 1) {
        fprintf(fptr, "createShaders(): fread() failed to read Tessellation Evaluation Shader file!.\n");
        vkResult = VK_ERROR_INITIALIZATION_FAILED;
        return (vkResult);
    } else {
        fprintf(fptr, "createShaders(): fread() succeed to read Tessellation Evaluation Shader file!.\n");
    }
    fclose(fp);
    fp = NULL;
    ...
    if(vkResult != VK_SUCCESS) {
        fprintf(fptr, "createShaders(): vkCreateShaderModule() for Tessellation Evaluation Shader Failed!.\n");
        return (vkResult);
    } else {
        fprintf(fptr, "createShaders(): vkCreateShaderModule() for Tessellation Evaluation Shader Successful!.\n");
    }
    ...
    fprintf(fptr, "createShaders(): Tessellation Evaluation Shader Module Created Successful!.\n");
...
- (VkResult) createPipeline {
    ...
    // MoltenVK/Metal has no isolines tessellation domain, so the tessellation
    // evaluation shader tessellates a quad collapsed onto a single curve
    // (see shader.tese). Rendering as wireframe draws that collapsed quad
    // as connected line segments instead of a filled sliver of triangles.
    // Degenerate v-direction triangles have inconsistent winding, so back-face
    // culling must stay off.
