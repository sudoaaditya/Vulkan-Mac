...
NSString *gszWindowTitle = @"macOS: Cube Texture - Vulkan";
...
- (VkResult) createVertexBuffer {
    ...
    if(bFoundMatchingMemoryType_vertex == VK_FALSE) {
        vkResult = VK_ERROR_OUT_OF_DEVICE_MEMORY;
        fprintf(fptr, "createVertexBuffer(): Failed to find suitable memory type for Vertex Buffer Normal!.\n");
        return (vkResult);
    } else {
        fprintf(fptr, "createVertexBuffer(): Suitable memory type found for Vertex Buffer Normal!.\n");
    }
...
- (VkResult) createDescriptorSetLayout {
    ...
    vkDescriptorSetLayoutBinding_array[0].stageFlags = VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT; // this binding is used in both vertex and fragment shaders
