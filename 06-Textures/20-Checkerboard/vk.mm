...
NSString *gszWindowTitle = @"macOS: Checkerboard Texture - Vulkan";
...
- (void) uninitialize {
    ...
    if(vkImageView_depth) {
        vkDestroyImageView(vkDevice, vkImageView_depth, NULL);
        fprintf(fptr, "uninitialize(): vkDestroyImageView() Succeed for Depth Image!\n");
        vkImageView_depth = VK_NULL_HANDLE;
    }
...
- (VkResult) createDescriptorPool {
    ...
    vkDescriptorPoolSize_array[0].descriptorCount = 2; // one uniform buffer per descriptor set (straight + oblique)

    // for texture sampler
    vkDescriptorPoolSize_array[1].type = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
    vkDescriptorPoolSize_array[1].descriptorCount = 2; // one texture sampler per descriptor set (straight + oblique)
