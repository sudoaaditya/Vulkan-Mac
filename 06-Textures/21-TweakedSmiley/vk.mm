...
NSString *gszWindowTitle = @"macOS: Tweaked Smiley Texture - Vulkan";
...
- (void) keyDown:(NSEvent *) event {
    ...
    switch (keyCode) {
        ...
        case '1':
            [self updateTexCoordBuffer:texCoord0];
            break;
        
        case '2':
            [self updateTexCoordBuffer:texCoord1];
            break;
        
        case '3':
            [self updateTexCoordBuffer:texCoord3];
            break;
...
- (VkResult) initialize {
    ...
    // Create Texture
    vkResult = [self createTexture:@"Smiley.png"];
    if(vkResult != VK_SUCCESS) {
