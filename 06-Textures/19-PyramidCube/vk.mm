...
NSString *gszWindowTitle = @"macOS: Textured Pyramid Cube - Vulkan";
...
- (VkResult) initialize {
    ...
    if(vkResult != VK_SUCCESS) {
     ...
     // Create Texture
    vkResult = [self createTexture:@"Vijay_Kundali.png" image:&vkImage_texture_cube memory:&vkDeviceMemory_texture_cube view:&vkImageView_texture_cube];
    if(vkResult != VK_SUCCESS) {
    ...
    vkResult = [self createTexture:@"Stone.png" image:&vkImage_texture_pyramid memory:&vkDeviceMemory_texture_pyramid view:&vkImageView_texture_pyramid];
    if(vkResult != VK_SUCCESS) {
...
- (VkResult) createTexture : (NSString*) textureFileName image: (VkImage*) vkImage_texture memory: (VkDeviceMemory*) vkDeviceMemory_texture view: (VkImageView*) vkImageView_texture {
    // variables
    VkResult vkResult = VK_SUCCESS;
    ...
    // Step 9: Create Sampler for Texture (shared across all textures, created only once)
    if(vkSampler_texture == VK_NULL_HANDLE) {
        ...
        fprintf(fptr, "createTexture(): Reusing existing Texture Sampler!.\n");
    }
...
- (VkResult) createDescriptorSetLayout {
    ...
    // Initialize Descriptor Set Bindings
    ...
    vkDescriptorSetLayoutBinding_array[1].binding = 1; // this 1 is the binding index, we will use this index in shader
    ...
    vkDescriptorSetLayoutCreateInfo.bindingCount = _ARRAYSIZE(vkDescriptorSetLayoutBinding_array); // uniform buffer + combined image sampler
...
- (VkResult) createDescriptorPool {
    ...
    vkDescriptorPoolSize_array[0].descriptorCount = 2; // pyramid + cube uniform buffers

    vkDescriptorPoolSize_array[1].type = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
    vkDescriptorPoolSize_array[1].descriptorCount = 2; // pyramid + cube texture samplers
    ...
    vkDescriptorPoolCreateInfo.maxSets = 4; // we have only one descriptor set
